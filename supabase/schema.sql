...
-- legitimate read. (The follow system and its additive read path were removed
-- July 2026 - the app is purely club-scoped; see the legacy cleanup at the end.)
...
-- Members only. (The follow system's additive read path was removed July 2026;
-- book rows are visible strictly to the book's club members.)
...
-- (The follow system's additive read path was removed July 2026; progress is
-- visible strictly to the book's club members.)
...
--
-- (The follow system's additive read path was removed July 2026; club
-- membership + the page gate below are the sole authority.)
--
...
-- club - never to non-members. There is no page gate: any member sees every
-- post in the club regardless of reading progress.
...
-- ============================================================================
-- LEGACY CLEANUP (safe no-ops on a fresh project)
-- ============================================================================
-- The follow system was removed in July 2026 - the app is purely club-scoped.
-- These drops clean up projects that predate the removal. They MUST run after
-- the policy definitions above: the old follow-era policies referenced
-- is_following(), and Postgres refuses to drop a function a live policy still
-- depends on, so the follow-free policy versions have to be in place first.
